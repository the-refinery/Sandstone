...
function _adodb_getcount(&$zthis, $sql,$inputarr=false,$secs2cache=0) 
{
	...
	if (is_set($rewritesql) && $rewritesql != $sql) {
		if ($secs2cache) {
						...
						$inputarr=false, $secs2cache=0) 
{
	...
	if (!is_set($nrows) || $nrows <= 0) $nrows = 10;
...
function &_adodb_pageexecute_no_last_page(&$zthis, $sql, $nrows, $page, $inputarr=false, $secs2cache=0) 
{
	...
	if (!is_set($page) || $page <= 1) {	// If page number <= 1, then we are at the first page
		$page = 1;
		$atfirstpage = true;
	}
...
function _adodb_getupdatesql(&$zthis,&$rs, $arrFields,$forceUpdate=false,$magicq=false,$force=2)
{
		...
		for ($i=0, $max=$rs->FieldCount(); $i < $max; $i++) {
			...
			if (adodb_key_exists($upperfname,$arrFields,$force)) {
				...
				else if (is_set($rs->fields[$upperfname])) $val = $rs->fields[$upperfname];
				else if (is_set($rs->fields[$field->name])) $val =  $rs->fields[$field->name];
				else if (is_set($rs->fields[strtolower($upperfname)])) $val =  $rs->fields[strtolower($upperfname)];
				else $val = '';
...
function adodb_key_exists($key, &$arr,$force=2)
{
	if ($force<=0) {
		...
		return (!empty($arr[$key])) || (is_set($arr[$key]) && strlen($arr[$key])>0);
	}

	if (is_set($arr[$key])) return true;
	## null check below
...
static $cacheCols;
	...
	if (is_string($rs)) {
		...
		if (is_set($rs->insertSig) && is_integer($cacheRS) && $cacheRS == $rs->insertSig) {
			$columns =& $cacheCols;
		} else {
			for ($i=0, $max=$rs->FieldCount(); $i < $max; $i++) 
...
function _adodb_debug_execute(&$zthis, $sql, $inputarr)
{
	...
	*/
	...
	if (is_set($zthis->dsnType)) $dbt .= '-'.$zthis->dsnType;
	if ($inBrowser) {
		if ($ss) {
...
function _adodb_backtrace($printOrArr=true,$levels=9999,$skippy=0)
{
	...
	$html =  (is_set($_SERVER['HTTP_USER_AGENT']));
	...
	foreach ($traceArr as $arr) {
		...
		if (is_set($arr['class'])) $s .= $arr['class'].'.';
		if (is_set($arr['args']))
