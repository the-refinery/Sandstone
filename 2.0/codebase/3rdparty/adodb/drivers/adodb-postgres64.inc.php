...
WHERE relkind in ('r','v') AND (c.relname='%s' or c.relname = lower('%s'))
 and c.relnamespace=n.oid and n.nspname='%s' 
	...
	function ServerInfo()
	{
		if (is_set($this->version)) return $this->version;
		...
		if (!empty($this->metaDefaultsSQL)) {
			...
			if ($rsdef) {
				while (!$rsdef->EOF) {
					...
					$s = $rsdef->fields['def'];
					if (strpos($s,'::')===false && substr($s, 0, 1) == "'") { /* quoted strings hack... for now... */
						$s = substr($s, 1);
		...
		if ($user || $pwd || $db) {
			...
			$db = adodb_addslashes($db);
		   	if ($str)  {
				...
				if (is_set($host[1])) $str .= " port=$host[1]";
...
class ADORecordSet_postgres64 extends ADORecordSet{
		...
		if (!$this->EOF) {
			...
			if ($this->_numOfRows < 0 || $this->_numOfRows > $this->_currentRow) {
				...
				if (is_array($this->fields) && $this->fields) {
					if (is_set($this->_blobArr)) $this->_fixblobs();
					return true;
				}
			}
			$this->fields = false;
	...
	function _fetch()
	{
		...
		if ($this->fields && is_set($this->_blobArr)) $this->_fixblobs();
