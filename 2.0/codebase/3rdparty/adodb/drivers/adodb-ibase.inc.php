...
class ADODB_ibase extends ADOConnection {
		...
		if (is_array($sql)) {
			...
			if (is_array($iarr)) {
				if  (ADODB_PHPVER >= 0x4050) { // actually 4.0.4
					if ( !is_set($iarr[0]) ) $iarr[0] = ''; // PHP5 compat hack
	...
	global $ADODB_FETCH_MODE;
		...
		while (!$rs->EOF) { //print_r($rs->fields);
			...
			if (is_set($rs->fields[1]) && $rs->fields[1]) {
				$fld->not_null = true;
			}				
			if (is_set($rs->fields[2])) {
		...
		//	case 35:$tt = 'TIMESTAMP'; break;
			...
			if ((is_set($rs->fields[5])) && ($fld->type == 'blob')) {
