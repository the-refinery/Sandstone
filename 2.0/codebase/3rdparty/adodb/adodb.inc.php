 ...
 if (!defined('_ADODB_LAYER')) {
	...
	*/
	...
	GLOBAL 
		...
		if (!is_set($ADODB_CACHE_DIR)) {
			$ADODB_CACHE_DIR = '/tmp'; //(is_set($_ENV['TMP'])) ? $_ENV['TMP'] : '/tmp';
		} else {
		 ...
		 */
		if (!is_set($ADODB_COUNTRECS)) $ADODB_COUNTRECS = true; 
	}
...
*/
	...
	*/
	...
	global $ADODB_FLUSH,$ADODB_OUTP;
		...
		if (is_set($_SERVER['HTTP_USER_AGENT']) || !$newline) echo $msg;
	...
	*/
	function &_rs2rs(&$rs,$nrows=-1,$offset=-1,$close=true)
	{
		...
		$rs2->fetchMode = is_set($rs->adodbFetchMode) ? $rs->adodbFetchMode : $rs->fetchMode;
		return $rs2;
	}
   ...
   */
	...
	* then we can scale to 12.8 million unique cached recordsets. Wow!
	...
	static $notSafeMode;
		...
		if (!is_set($notSafeMode)) $notSafeMode = !ini_get('safe_mode');
	...
	function &CacheExecute($secs2cache,$sql=false,$inputarr=false)
	{
		...
		// ok, set cached object found
			...
			if ($this->debug){ 
				...
				$inBrowser = is_set($_SERVER['HTTP_USER_AGENT']);
	...
	*/
	 ...
	 */
		...
		global $ADODB_FETCH_MODE;
			...
			if ($this->metaDatabasesSQL) {
				...
				if (is_set($savem)) $this->SetFetchMode($savem);
	...
	global $ADODB_FETCH_MODE;
		...
		if (!empty($this->metaColumnsSQL)) {
			...
			while (!$rs->EOF) { //print_r($rs->fields);
				...
				if (is_set($rs->fields[3]) && $rs->fields[3]) {
	...
	function UnixTimeStamp($v)
	{
		...
		// h-m-s-MM-DD-YY
		if (!is_set($rr[5])) return  adodb_mktime(0,0,0,$rr[2],$rr[3],$rr[1]);
	...
	{
		if (!is_set($v)) return $this->emptyTimeStamp;
		# strlen(14) allows YYYYMMDDHHMMSS format
   ...
   */
	...
	*/
	...
	static $typeMap = array(
		...
		$tmap = false;
		$t = strtoupper($t);
		$tmap = (is_set($typeMap[$t])) ? $typeMap[$t] : 'N';
		switch ($tmap) {
		case 'C':
	...
	class ADORecordSet_array extends ADORecordSet
	{
		...
		function _initrs()
		{
			...
			$this->_numOfFields =(is_set($this->_fieldobjects)) ?
		...
		function Fields($colname)
		{
			$mode = is_set($this->adodbFetchMode) ? $this->adodbFetchMode : $this->fetchMode;
			
			if ($mode & ADODB_FETCH_ASSOC) {
				if (!is_set($this->fields[$colname])) $colname = strtolower($colname);
				return $this->fields[$colname];
			}
			if (!$this->bind) {
				$this->bind = array();
		...
		if ($at = strpos($db,'://')) {
			...
			if (PHP_VERSION < 5) $dsna = @parse_url($db);
				...
				if (strncmp($db,'pdo',3) == 0) {
					...
					if (sizeof($sch)>1) {
						$dsna['host'] = is_set($dsna['host']) ? rawurldecode($dsna['host']) : '';
			...
			if (!$db) return $false;
			$dsna['host'] = is_set($dsna['host']) ? rawurldecode($dsna['host']) : '';
			$dsna['user'] = is_set($dsna['user']) ? rawurldecode($dsna['user']) : '';
			$dsna['pass'] = is_set($dsna['pass']) ? rawurldecode($dsna['pass']) : '';
			$dsna['path'] = is_set($dsna['path']) ? rawurldecode(substr($dsna['path'],1)) : ''; # strip off initial /
			
			if (is_set($dsna['query'])) {
				$opt1 = explode('&',$dsna['query']);
				foreach($opt1 as $k => $v) {
					$arr = explode('=',$v);
					$opt[$arr[0]] = is_set($arr[1]) ? rawurldecode($arr[1]) : 1;
				}
			} else $opt = array();
		}
	/*
	 ...
	 */
		if (!empty($ADODB_NEWCONNECTION)) {
			...
			if (!is_set($ADODB_LASTDB)) $ADODB_LASTDB = '';
			if (empty($db)) $db = $ADODB_LASTDB;
			
			if ($db != $ADODB_LASTDB) $db = ADOLoadCode($db);
			
			if (!$db) {
				if (is_set($origdsn)) $db = $origdsn;
				if ($errorfn) {
					// raise an error
					$ignore = false;
		...
		if ($obj) {
			...
			if (is_set($dsna)) {
				if (is_set($dsna['port'])) $obj->port = $dsna['port'];
				foreach($opt as $k => $v) {
					switch(strtolower($k)) {
					case 'new':
