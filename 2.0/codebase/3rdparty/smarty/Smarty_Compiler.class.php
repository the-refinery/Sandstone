...
class Smarty_Compiler extends Smarty {
    ...
    function _compile_tag($template_tag)
    {
        ...
        $tag_command = $match[1];
        $tag_modifier = is_set($match[2]) ? $match[2] : null;
        $tag_args = is_set($match[3]) ? $match[3] : null;
    ...
    function _compile_compiler_tag($tag_command, $tag_args, &$output)
    {
         ...
         */
        if (is_set($this->_plugins['compiler'][$tag_command])) {
            $found = true;
    ...
    function _compile_block_tag($tag_command, $tag_args, $tag_modifier, &$output)
    {
         ...
         * or loaded from a plugin file.
         */
        if (is_set($this->_plugins['block'][$tag_command])) {
            $found = true;
    ...
    function _compile_custom_tag($tag_command, $tag_args, $tag_modifier, &$output)
    {
         ...
         * or loaded from a plugin file.
         */
        if (is_set($this->_plugins['function'][$tag_command])) {
            $found = true;
    ...
    function _compile_section_start($tag_args)
    {
        ...
        if (!is_set($attrs['show']))
            $output .= "{$section_props}['show'] = true;\n";

        if (!is_set($attrs['loop']))
        ...
        if (!is_set($attrs['step']))
            $output .= "{$section_props}['step'] = 1;\n";

        if (!is_set($attrs['start']))
        ...
        if (!is_set($attrs['start']) && !is_set($attrs['step']) && !is_set($attrs['max'])) {
    ...
    function _compile_if_tag($tag_args, $elseif = false)
    {
        ...
        if(is_set($token_count['(']) && $token_count['('] != $token_count[')']) {
        ...
        for ($i = 0; $i < count($tokens); $i++) {
            ...
            switch (strtolower($token)) {
                ...
                default:
                    ...
                    } elseif(preg_match('~^' . $this->_var_regexp . '$~', $token) && (strpos('+-*/^%&|', substr($token, -1)) === false) && is_set($tokens[$i+1]) && $tokens[$i+1] == '(') {
    ...
    function _compile_arg_list($type, $name, $attrs, &$cache_code) {
        ...
        if (is_set($type) && is_set($name)
            && is_set($this->_plugins[$type])
            && is_set($this->_plugins[$type][$name])
    ...
    function _parse_is_expr($is_arg, $tokens)
    {
        ...
        switch ($expr_type) {
            case 'even':
                if (is_set($tokens[$expr_end]) && $tokens[$expr_end] == 'by') {
                    $expr_end++;
            ...
            foreach ($_indexes as $_index) {
                if (substr($_index, 0, 1) == '[') {
                    ...
                    if (is_numeric($_index)) {
                        ...
                        $_var_section_prop = is_set($_var_parts[1]) ? $_var_parts[1] : 'index';
    ...
    function _parse_conf_var($conf_var_expr)
    {
        ...
        $var_ref = $parts[0];
        $modifiers = is_set($parts[1]) ? $parts[1] : '';
    ...
    function _parse_modifiers(&$output, $modifier_string)
    {
        ...
        for ($_i = 0, $_for_max = count($_modifiers); $_i < $_for_max; $_i++) {
            ...
            if($_modifier_name == 'default') {
                ...
                if(is_set($_modifier_args[0]) && substr($_modifier_args[0], 0, 1) == '$') {
    ...
    {
        if (!is_set($this->_plugin_info[$type])) {
            $this->_plugin_info[$type] = array();
        }
        if (!is_set($this->_plugin_info[$type][$name])) {
    ...
    function _compile_smarty_ref(&$indexes)
    {
        ...
        if (is_set($_max_index) && count($indexes) > $_max_index) {
    ...
    function _compile_plugin_call($type, $name) {
        if (is_set($this->_plugins[$type][$name])) {
            /* plugin loaded */
    ...
    function _push_cacheable_state($type, $name) {
        $_cacheable = !is_set($this->_plugins[$type][$name]) || $this->_plugins[$type][$name][4];
        if ($_cacheable
            || 0<$this->_cacheable_state++) return '';
        if (!is_set($this->_cache_serial)) $this->_cache_serial = md5(uniqid('Smarty'));
