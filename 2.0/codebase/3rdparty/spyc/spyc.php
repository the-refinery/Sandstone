  ...
  class Spyc {
        ...
        if ($this->_inBlock === false && empty($ifchk)) {
          ...
          if ($this->_lastIndent == $node->indent) {
            ...
            if ($this->_inBlock === true) {
              ...
              if (is_set($this->_allNodes[$this->_lastNode])) {
            ...
            if ($this->_inBlock === true) {
              ...
              if (is_array($parent->data)) {
                // if (is_set ($parent->data[key($parent->data)]))
          ...
          if ($this->_inBlock === false) {
            ...
            if (
              ( (is_array($node->data)) &&
                is_set($node->data[key($node->data)]) &&
     ...
     */
     private function _makeReferences(&$z) {
      ...
      // It has a reference
      } elseif (is_set($z->refKey)) {
        if (is_set($this->ref[$z->refKey])) {
          $key           = key($z->data);
     ...
     */
     private function _array_kmerge($arr1,$arr2) {
      ...
      foreach($keys as $key) {
        ...
        if (is_set($ret[$key]) and is_int($key)) $ret[] = $val; else $ret[$key] = $val;
      }
      return $ret;
    }
  }
