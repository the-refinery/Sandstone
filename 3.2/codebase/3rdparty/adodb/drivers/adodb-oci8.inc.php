			 ...
			 if (is_array($inputarr)) {
			 	foreach($inputarr as $k => $v) {
					if (is_array($v)) {
						...
						$len = -1;
						if ($v === ' ') $len = 1;
						if (is_set($bindarr)) {	// is prepared sql, so no need to ocibindbyname again
							$bindarr[$k] = $v;
...
class ADORecordset_oci8 extends ADORecordSet {
	...
	function MetaType($t,$len=-1)
	{
		...
		case 'NVARCHAR':
		case 'NVARCHAR2':
				 if (is_set($this) && $len <= $this->blobSize) return 'C';
