	...
	public function DItraceSimpleAsString()
	{
		...
		foreach($this->_diCallStack as $traceItem)
		{
			if (is_set($traceItem['class']))
			{
				if ($index == 0)
				{
					$lineItem = "<tr id=\"summary{$index}\" onclick=\"SetCurrent({$index});\" class=\"Current\" >";
				}
				else
				{
					$lineItem = "<tr id=\"summary{$index}\" onclick=\"SetCurrent({$index});\" >";
				}
				$lineItem .= "<td class=\"stackorder\">#{$index}</td>";
				$lineItem .= "<td class=\"linenumber\" onclick=\"ShowDetail({$index});\">{$this->LineNumberAsString($traceItem['line'])}</td>";
				$lineItem .= "<td><strong>{$traceItem['class']}{$traceItem['type']}{$traceItem['function']}()</strong><br />
									<i>{$traceItem['file']}</i>
									</td>";
				$lineItem .= "</tr>";
			}
	...
	public function DItraceDetailAsString()
	{
		...
		foreach($this->_diCallStack as $traceItem)
		{
			if (is_set($traceItem['class']))
			{
				if ($index == 0)
				{
					$lineItem = "<tr id=\"detail{$index}\" onclick=\"SetCurrent({$index});\" class=\"Current\" >";					
				}
				else
				{
					$lineItem = "<tr id=\"detail{$index}\" onclick=\"SetCurrent({$index});\" >";
				}
				
				$lineItem .= "<td class=\"stackorder\">#{$index}</td>";
				$lineItem .= "<td class=\"linenumber\" onclick=\"ShowSummary({$index});\">{$this->LineNumberAsString($traceItem['line'])}</td>";
				$lineItem .= "<td><strong>{$traceItem['class']}{$traceItem['type']}{$traceItem['function']}</strong>{$traceItem['fullargs']}<br />
									</td>";
				$lineItem .= "</tr>";
			}
			else 
