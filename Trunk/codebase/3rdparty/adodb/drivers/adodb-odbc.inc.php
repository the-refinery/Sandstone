...
class ADODB_odbc extends ADOConnection {
	...
	global $php_errormsg;
		...
		if (is_set($php_errormsg)) $php_errormsg = '';
		if ($this->curmode === false) $this->_connectionID = odbc_connect($argDSN,$argUsername,$argPassword);
		else $this->_connectionID = odbc_connect($argDSN,$argUsername,$argPassword,$this->curmode);
		$this->_errorMsg = is_set($php_errormsg) ? $php_errormsg : '';
		if (is_set($this->connectStmt)) $this->Execute($this->connectStmt);
	...
	function ServerInfo()
	{
		...
		if (!empty($this->host) && ADODB_PHPVER >= 0x4300) {
			...
			if (!is_set($rez['version'])) $rez['version'] = '';
			return $rez;
		} else {
