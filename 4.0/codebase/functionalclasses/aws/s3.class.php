...
class s3
{
	...
	// -----------------------------------------
	// your API key ID
	var $keyId = "*** ENTER API KEY ID HERE ***";
	// your API Secret Key
	var $secretKey = "*** ENTER SECRET KEY HERE ***";
	// -----------------------------------------
	// -----------------------------------------
	...
	*/
	function sendRequest ($resource, $verb = NULL, $objectdata = NULL, $acl = NULL, $contentType = NULL, $metadata = NULL)
	{
		...
		if (strlen($contenttypestring) > 0) {
			$this->debug_text("Setting content type to $contenttypestring");
			$req->addHeader("CONTENT-TYPE", $contenttypestring);
		}
		...
		$req->addHeader("Authorization", "AWS " . Application::Registry()->AWSaccessKeyID . ":" . $signature);
	...
	*/
	function putObject ($objectname, $objectdata, $bucketname = NULL, $acl = NULL, $contentType = NULL, $metadata = NULL) {
		...
		{
			$objectname = Application::License()->AccountID . "/" . $objectname;
		}
	...
	*/
	...
	function GenerateFileURL($ObjectKey, $Expires = 300)
	{
		...
		$urlSignature = rawurlencode($signature);

		$returnValue = "https://s3.amazonaws.com/{$bucket}/{$ObjectKey}";
		$returnValue .= "?AWSAccessKeyId=" . Application::Registry()->AWSaccessKeyID;
		$returnValue .= sprintf('&Expires=%u', $time);
		$returnValue .= "&Signature={$urlSignature}";
