...
class SearchEntity extends EntityBase
{
	...
	protected function CalculatePropertiesRank($SearchTerm, $ResultObject)
	{
		...
		foreach ($this->SearchableProperties as $tempProperty)
		{
			...
			$propertyValue = $ResultObject->$propertyName;

			if ($propertyValue instanceof DIarray || is_array($propertyValue))
			{
				foreach ($propertyValue as $tempValue)
				{
					$returnValue += $this->CalculatePropertyValueRank($tempValue, $SearchTerm, $tempProperty);
				}
			}
			else
			{
				//Single value
				$returnValue += $this->CalculatePropertyValueRank($propertyValue, $SearchTerm, $tempProperty);
			}
		}
	...
	protected function CalculatePropertyValueRank($Value, $SearchTerm, $SearchProperty)
	{

		//Cast it to all lowercase
		...
		{
			//Dead Hit!
			$returnValue = $SearchProperty->MatchWeight;
		}
		else
		{
			//Is this a wildcard match?
			$matchCount = substr_count($Value, $SearchTerm);

			if ($matchCount > 0)
			{
				$returnValue = $matchCount * $SearchProperty->WildcardWeight;
			}
		}
	...
	protected function CalculateTagRank($SearchTerm, $ResultObject)
	{
    	...
    	foreach($ResultObject->Tags as $tempTag)
		{
