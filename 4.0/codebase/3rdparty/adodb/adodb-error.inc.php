...
global $ADODB_LANG,$ADODB_LANG_ARRAY;
	...
	if (is_set($ADODB_LANG_ARRAY['LANG']) && $ADODB_LANG_ARRAY['LANG'] == $ADODB_LANG) ;
	else {
		include_once(ADODB_DIR."/lang/adodb-$ADODB_LANG.inc.php");
    }
	return is_set($ADODB_LANG_ARRAY[$value]) ? $ADODB_LANG_ARRAY[$value] : $ADODB_LANG_ARRAY[DB_ERROR];
}
...
function adodb_error($provider,$dbType,$errno)
{
	...
	//print_r($map);
	//var_dump($errno);
	if (is_set($map[$errno])) return $map[$errno];
	return DB_ERROR;
}
