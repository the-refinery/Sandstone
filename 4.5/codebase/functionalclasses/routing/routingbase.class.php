...
class RoutingBase extends Module
{
	...
	protected function AddRulesToPool($Rules)
	{
		foreach ($Rules as $key=>$value)
		{
			switch (strtolower($value['resource']))
			{
				case 'rest':
					$this->MakeRestful($value);
					break;
	...
	protected function MakeRestful($RuleData)
	{
		$pluralClass = $this->Pluralize($RuleData['class'], $RuleData['plural']);
    ...
    static $count;
		...
		$primarykey = $this->DeterminePrimaryKeyField($class, $RuleData['primarykey']);

		$eventParameters['primarykey'] = $primarykey;
	...
	protected function DeterminePrimaryKeyField($Class, $AlternatePrimaryKey = null)
	{
		...
		{
			$returnValue = $AlternatePrimaryKey;
		}
		else
		{
			$returnValue = $Class . 'id';
		}

		return $returnValue;
	}

	protected function Pluralize($Class, $AlternatePluralization = null)
	{
		if ($AlternatePluralization)
		{
			$returnValue = $AlternatePluralization;
		}
		else
		{
			$returnValue = $Class . 's';
